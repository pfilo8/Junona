...
F1 Score to średnia harmoniczna precyzji oraz czułości. 
...
W celu wprowadzenia potrzebnych miar skuteczności wrażliwych na koszt potrzebujemy zdefiniować macierz kosztu, która w odpowiednich komórkach tabeli ... Warto zaznaczyć, że koszt klasyfikacji może różnić się między poszczególnymi obserwacjami.
\begin{table}[h]
	\begin{center}
		\begin{tabular}{c|c|c}
			\multirow{2}{4em}{} & Stan pozytywny & Stan negatywny \\
...
\end{table}
% TODO: Popisać o kosztach ...
% TODO: Rozbić na cztery przypadki
...
\end{itemize}{}
Wartość $ f_{a}(\es)$ możemy rozumieć jako przypadek naiwnego modelu, który wszystkim obserwacjom przyznaje wartość $a$. Natomiast $ \text{Koszt}_n $ oznacza wybranie naiwnego klasyfikatora, który generuje mniejsze koszty. Ostatecznie oszczędności możemy rozumieć jako procentową wartość o ile testowany model jest lepszy od naiwnego klasyfikatora.
...
\begin{table}[h]
	\begin{center}
		\begin{tabular}{c|c|c}
			...
			& $y_i = 1$            & $y_i = 0$ \\
			\hline
			Predykcja pozytywna & \multirow{2}{4em}{\centering $\text{C}_{\text{a}}$} & \multirow{2}{4em}{\centering $\text{Amt}_{\text{i}}$} \\
			$c_i = 1$         &                    &                    \\
			\hline
			Predykcja negatywna & \multirow{2}{4em}{\centering $\text{C}_{\text{a}}$} & \multirow{2}{4em}{\centering 0} \\
			$c_i = 0$         &                    &                    \\
		\end{tabular}
	\end{center}

...
\begin{figure}[h]
	...
	\caption{Schemat płatności kartą płatniczą.}
...
\end{itemize}
Warto zaznaczyć, że implementacja w bibliotece \textit{sklearn} w Pythonie wykorzystuje zoptymalizowaną wersję algorytmu CART, który nie obsługuje zmiennych kategorycznych. Poniżej prezentujemy teorię matematyczną stojącą za implementacją tego algorytmu \cite{sklearn_api}.
...
Możliwe jest także stworzenie modeli typu \textit{ensemble}, w których klasyfikatorem bazowym byłoby omawiane drzewo decyzyjne wrażliwe na koszt, natomiast wykracza to poza niniejszą pracę. Szczegóły takiego podejścia są opisane w odpowiedniej pracy A. Bahnsensa \cite{alej2015ensemble}.
